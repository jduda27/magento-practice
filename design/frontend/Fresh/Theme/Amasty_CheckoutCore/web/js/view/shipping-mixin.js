  ...
  function (
    ...
    return function (Shipping) {
      return Shipping.extend({
        ...
        getVisibleUserfield:function (){
          var show_userclass = window.localStorage.getItem('show_userclass');
          var is_commercial = window.localStorage.getItem('is_commercial');
          /* on first load bassed on market sector */
           var iscomercial = window.localStorage.getItem('iscommercial');
           /* on first load bassed on market sector */
          var is_specifier = window.localStorage.getItem('is_specifier');
          var project_classification = window.localStorage.getItem('project_classification');
          
         
          setTimeout(function () {
            if (is_commercial!= null) {
                $(".switch-one input[value='"+is_commercial+"']").trigger('click');
                $(".switch input[value='"+is_specifier+"']").trigger('click');
                $('select[name="project_classification"] option[value="'+project_classification+'"]').prop('selected', true);
            }else{
              $(".switch-one input[value='"+iscomercial+"']").trigger('click');
            }
           
          },1000);

          if(show_userclass == 1){
            show_userclass = false;
            $('.checkout-index-index #shipping').addClass('hide_projectSection');
          }else{
            show_userclass = true;
            $('.checkout-index-index #shipping').removeClass('hide_projectSection');
          }
          return show_userclass;
        }
      });
    };
  }
